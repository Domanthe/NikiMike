...
public class Decrypt {
	...
	private void loadKeyStore() {
		try {
			...
			this.keyStore.load(new FileInputStream(this.ksPath),
	...
	private void extractConfiguration(String configFilePath) {
		...
		try {
			...
			ksPath = fileOutputStream.getProperty("KeyStoreFile");
			ksAlias = fileOutputStream.getProperty("KeyStoreAlias");
			IV = hexToByteArray(fileOutputStream.getProperty("IV"));
			cipherFileLocation = fileOutputStream.getProperty("EncryptedFileLocation");
	...
	public Boolean validateSignature() {
		...
		try {
			...
			// // Digest message
			// MessageDigest messageDigest = MessageDigest.get
			// messageDigest.update(this.decryptedText);
			// byte[] clearTextDigest = messageDigest.digest();

			PrivateKeyEntry entry = (PrivateKeyEntry) keyStore.getEntry(ksAlias,
					new KeyStore.PasswordProtection(KEY_STORE_PASSWORD.toCharArray()));
			PublicKey publicKey = entry.getCertificate().getPublicKey();

			// Initiate Signature object
			Signature signatureObj = Signature.getInstance(SignatureEncryptAlgo,
					SignatureEncryptAlgoProvider);
			...
			// Save the signature
			isValid = signatureObj.verify(cipherSignature);
		} catch (Exception e) {
	 ...
	 * 
	 * @param hexString
	 *            the string in hex we wish to convert
